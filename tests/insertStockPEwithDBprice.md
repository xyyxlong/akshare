...
*   **核心目的**：在无法使用 `akshare` 接口（如 `ak.stock_a_indicator_lg`）的情况下，基于**本地数据库**的历史行情和财务数据，自主计算股票的估值指标（PE/PB/PS/Dividend）。
...
该脚本采用**单体数据库直连架构**，直接读取原始数据并写入结果表。

| 组件 | 技术栈 | 说明 |
| :--- | :--- | :--- |
| **数据库** | MySQL (PyMySQL) | 读取源表：`stock_historical_data`, `dividend_info`, `stock_financial_reports`写入目标表：`stock_pe_history` |
...
*   **滚动市盈率 (PE_TTM)**:
    ...
    *   **代码逻辑 (`calculate_eps_ttm`)**:
        1.  **最新季报**：获取截止计算日（如2026-05-20）最新的财报数据。
        2.  **上年年报**：获取上一年度（如2025-12-31）的年报 EPS。
        3.  **上年同期**：获取与最新季报相同时间段的去年数据（如最新是Q1，则取去年Q1）。
        4.  **计算**：通过加减法消除重复计算部分，得出最近四个季度的真实盈利。
...
3.  **市值估算**：
    *   代码中的 `estimate_market_cap` 方法目前是**占位符**（返回 None）。这意味着 `total_mv` 字段在当前版本中可能无法写入有效数据，需要依赖外部实现。
4.  **日期处理**：
...
**代码质量评价**：
*   **优点**：逻辑清晰，对 TTM 的计算处理得当，考虑了分红的财年归属问题，日志记录完善。
*   **缺点**：硬编码较多（如日期范围写死在 `main` 函数中），`estimate_market_cap` 功能缺失，依赖特定的数据库表结构。

**针对当前时间 (2026-05-21) 的建议**：
1.  **运行检查**：由于脚本中的 `end_date` 写死为 `'2026-5-20'`，今天是 5月21日，建议更新日期范围以包含最新的交易日数据。
2.  **市值计算修复**：如果业务需要 `total_mv` 字段，需要补充 `estimate_market_cap` 的实现逻辑（通常为：流通股本 * 股价，或从专门的股本表中获取总股本）。
3.  **断层处理**：正如代码注释所述，由于数据源切换，2025年7月前后的数据可能存在差异，建议在前端展示时增加数据源切换的提示。